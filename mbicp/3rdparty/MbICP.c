...
//  Alex: I change this constant to make it a parameter
float MAXLASERRANGE = 8.1;

// ************************
// Some static internal variables
// Cos and sin values for the scan angles
// static float cosAng[361];
// static float sinAng[361];
...
void Init_MbICP_ScanMatching(float Bw, float Br,
...
static int EStep()
{
  ...
  LMET2=params.LMET*params.LMET;
	...
	for (i=0; i<ptosNew.numPuntos; i++){
		...
		car2pol(&ptosNewRef.laserC[ptosNewRef.numPuntos],&ptosNewRef.laserP[ptosNewRef.numPuntos]); /** ojo **/
	...
	if (params.ProjectionFilter==1){
		for (i=1;i<ptosNewRef.numPuntos;i++){
			if (ptosNewRef.laserP[i].t>=ptosNewRef.laserP[cnt-1].t){ /* punto visible */
