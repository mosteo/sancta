...
package body Sancta.Ctree.Component.Scluster_Hungarian is
...
--              Log ("Base " & Base_Name & " removed.", Debug, Log_Section);
            ...
            Ass_1 := Assigner.Hungarian.Assign (Ass_0.Get_Agents,
                                                Jobs,
                                                Costs,
                                                Scluster_Tree);
   ...
   begin
      ...
      if This.Ready then
         ...
         begin
            ...
            if This.Exists (Option_Agent_Name) then
               declare
                  Bot : constant Located_Agent.Object'Class :=
                          Located_Agent.Object'Class
                            (Bots.Get_Agent (This.Option (Option_Agent_Name)));
               begin
                  if Bot.Has_Tasks then
                     This.Output
                       (Provides_Pose_Goal,
                        Ctypes.Pose'
                          (Pose => Posed.Object (Bot.Get_First_Task).Pose));
                  else
                     This.Output
                       (Provides_Pose_Goal,
                        Ctypes.Pose'(Pose => (0.0, 0.0, 0.0)));
--                       This.Output
--                         (Provides_No_Goal,
--                          Ctypes.Bool'(Value => False));
                  end if;
               end;
            end if;
         end;
