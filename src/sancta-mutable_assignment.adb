...
package body Sancta.Mutable_Assignment is
   ...
   function To_String is new Conversions.Decimal_To_Str (Sancta.Costs);
   ...
   begin
      return L.Cost < R.Cost
            or else (L.Cost = R.Cost and then L.Agent < R.Agent);
   end "<";
   ...
   is
   begin
      This.Context.Ref.all.Mutations.Append
        ((Doer => Mutator, Undoer => Undoer, Weight => Weight, Prob => 0.0));
      ...
      begin
         ...
         for I in M.First .. M.Last loop
            ...
            M.Vector (I).Prob :=
              Optimization.Annealing.Probability (Acum_Weight / Total_Weight);
            if I = M.First then
               Log
                 ("Mutation" & I'Img & ": P = " & S (M.Vector (I).Prob, 5),
                  Debug,
   ...
   procedure Add_Or_Contexts
     (This : in out Object;
      Node : in Sancta.Plan.Subplan)
   is
      use Sancta.Plan_Node;
      Parent : constant Sancta.Plan_Node.Node_Access := Get_Parent (Node);
   begin
      if Parent /= null then
         if Get_Kind (Parent) = Or_Node then
            if not This.Contexts.Contains (Or_Key (Get_Id (Parent))) then
               ...
               declare
                  C : Or_Context := (Solution_Context with Node => Parent,
                    Branch => Node);
               begin
               ...
               begin
                  if C.Branch /= Node then
                     C.Branch := Node;
                     Log
                       ("Add_Or_Nodes: Corrected Or Branch taken.",
                        Debug,
                        Detail_Section);
                  end if;
               end;
   ...
   is
      ...
      is
      begin
         --  Log ("Adding " & S (Context.Key) & " to bag " & String (Key),
         --  Always);
   ...
   procedure Adjust_Chain_Inserting
     (This        : in out Object;
   ...
   begin
      if (Aft /= null and then Bfr /= null and then Aft.Next /= Bfr.Job)
        or else (Aft /= null
                and then Bfr = null
                and then Aft.Next /= No_Task)
        or else (Bfr /= null
                and then Aft = null
                and then Bfr.Prev /= No_Task)
      then
   ...
   procedure Adjust_Chain_Removing
     (This : in out Object;
      Job  : in Task_Context_Ptr)
   is
   ...
   begin
      return Solution_Context_Key'(Key ("A:" & Name));
   end Agent_Key;
   ...
   procedure Create_Empty_Bag (This : in out Object; Key : in Bag_Key) is
      Empty_Bag : Solution_Context_Bags.Object (First => 1);
   begin
      Empty_Bag.Set_Context
        ((Key => +String (Key), Parent => This'Unchecked_Access));
   ...
   procedure Create_Some_Solution
     (This      : in out Object;
   ...
   procedure Common_Dump (This : in Solution_Context'Class) is
      ...
      begin
         Log
           (Attribute_Maps.Key (I)'Img & " = " & Attribute_Maps.Element (I),
            Always);
      end Dump_Attrs;
      ...
      begin
         Log
           ("Bag idx for " & String (Key (I)) & " =" & Element (I)'Img,
            Always);
      end Dump_Bags;
   begin
   ...
   procedure Do_Heuristic_1 (This : in out Object; Undo : out Undo_Info) is
      U : Undo_Internal (From_Scratch);
      A : constant Sancta.Assignment.Object := This.To_Assignment;
   begin
      U.Ass         := A;
      ...
      declare
         use Sancta.Assignment;
         New_Assignment : constant Sancta.Assignment.Object :=
           Sancta.Assigner.Greedy_Minmax_Exhaustive.Assign
              ((Sancta.Assigner.Object with Keep_Order => True),
      ...
      begin
         ...
         --  Note: here criterion will not be used since there are no new
         --  tasks.
      end;
   end Do_Heuristic_1;
   ...
   procedure Do_Identity (This : in out Object; Undo : out Undo_Info) is
      pragma Unreferenced (This);
      U : constant Undo_Internal := (Identity, +"IDENTITY");
   begin
      ...
      begin
         ...
         begin
            if Expensive_Checks then
               ...
               if Acm.Element (This.Agent_Costs.Find (New_Owner)) /=
                  Cost
               then
                  Log
                    ("Eval: " &
                     ...
                     "Stor: " &
                     To_String
                        (Acm.Element (This.Agent_Costs.Find (New_Owner)),
                         5),
                     Always);
                  raise Program_Error;
               end if;
   ...
   procedure Do_Move_Task
     (This        : in out Object;
      ...
      New_Owner   : in Agent_Id)
   is
      Src_Copy : constant Task_Context := Task_Context (Src.all);
   begin
      This.Do_Remove_Task (Src);
      Do_Insert_Task (This, After_This, Src_Copy, Before_This, New_Owner);
   ...
   begin
      Log
        ("Descending into " & Get_Kind (Node)'Img & " " & Get_Id (Node),
         Debug,
         Detail_Section);
      case Get_Kind (Node) is
         ...
         when And_Node =>
            declare
               Children : constant Node_Vectors.Vector := Get_Children (Node);
            begin
         ...
         when Or_Node =>
            ...
            begin
               This.Descend_Removing
                 (Or_Context (Branch_Ptr.all).Branch,
                  Undo);
      ...
      begin
         ...
         Update_Costs_Removing
           (This,
            ...
            Former_Owner       => Get_Attribute (Job.all'Access, Owner));
   ...
   function Evaluate (This : in Object) return Optimization.Cost is
      C : constant Sancta.Costs := This.Evaluate;
   begin
   ...
   function Get_Attribute
     (Context : not null access Solution_Context'Class;
   ...
   function Get_Attribute
     (Context : in Solution_Context'Class;
      Attr    : in Solution_Context_Attributes)
      return    String
   is
      use Attribute_Maps;
   ...
   function Is_Sane (This : in Object) return Boolean is
      procedure Check_Or_Parents (Node : in Sancta.Plan.Subplan) is
         use Sancta.Plan_Node;
      begin
         if Get_Kind (Node) = Or_Node
           and then not This.Contexts.Contains (Or_Key (Get_Id (Node)))
         then
...
"Compound or finished task is assigned";
               else
            ...
            begin
               if Get_Kind (Oc.Branch) = Task_Node
                 and then not Get_Expanded (Oc.Branch)
               then
                  if not This.Contexts.Contains
                           (Task_Key (Get_Task (Oc.Branch).all.Get_Id))
                  then
                     Log
                       ("Missing task context for task " &
            ...
            raise Constraint_Error with "Unexpected context kind: " &
                                        External_Tag (X'Tag);
         end if;
      end Check_Contexts;
   ...
   procedure Moving_Solution_Context
     (Context    : in out Solution_Context_Key;
      Bag        : in out Bag_Context;
      Prev, Curr : in Integer)
   is
      use Index_Maps;
      C : constant Solution_Context_Ptr := Bag.Parent.all.Ptr (Context);
   begin
      pragma Assert
        (Element (C.Bag_Indexes.Find (Bag_Key (+Bag.Key))) = Prev);
   ...
   procedure Mutate (This : in out Object) is
      ...
      M    : Mutation_Vectors.Object renames This.Context.Ref.Mutations;
   begin
      --  This.Debug_Dump_Contexts;
      ...
      if This.Context.Ref.Agents.Is_Empty
        or else This.Context.Ref.Plan.Is_Empty
      then
      ...
      for I in M.First .. M.Last loop
         if Luck <= M.Vector (I).Prob then
            ...
            Log
              ("Performing mutation" & I'Img & "; Valid: " & This.Valid'Img,
               Debug,
               Section => Detail_Section);
            ...
            Log
              ("Mutated: " &
               (+This.Undo.Handle.Get.Description) &
               "; Valid: " &
               This.Valid'Img,
               Debug,
               Section => Detail_Section);
      ...
      for I in M.First .. M.Last loop
         Log
           ("Prob. for mut." & I'Img & ":" & M.Vector (I).Prob'Img,
            Error,
            Log_Section);
      end loop;
   ...
   function Normalize
     (Old_Cost, New_Cost : in Optimization.Cost;
   ...
   begin
      if New_Cost < Old_Cost then
         ...
         return (Acceptability (Old_Cost / New_Cost) * Acceptability (Temp)) **
                0.75; -- We increase the wildlity a bit.
      end if;
   exception
   ...
   function Ptr
     (This : in Object;
      Key  : in Task_Context_Key)
      return Task_Context_Ptr
   is
      Result : Task_Context_Ptr;
      procedure Do_It
        (Key : Solution_Context_Key;
         C   : Solution_Context'Class)
      is
         pragma Unreferenced (Key);
      begin
   ...
   begin
      if Key = No_Task_Key then
         ...
         Solution_Context_Maps.Query_Element
           (This.Contexts.Find (Solution_Context_Key (Key)),
            Do_It'Access);
         return Result;
      end if;
   end Ptr;
   ...
   procedure Reevaluate_Agent_Cost
     (This  : in out Object;
      Agent : in Agent_Id;
      Cost  : out Costs)
   is
      C     : Sancta.Cost_Cache.Object'Class renames This.Context.Ref.Costs.
        Ref.all;
      Total : Costs := 0.0;
   ...
   procedure Reevaluate_Minmax (This : in out Object; Cost : out Costs) is
   ...
   procedure Reevaluate_Minsum (This : in out Object; Cost : out Costs) is
      procedure Ev (I : Agent_Maps.Cursor) is
         Local : Costs;
      begin
   ...
   begin
      ...
      if This.Context.Ref.Agents.Is_Empty then
         ...
         Reassign_Tasks
           (This,
            Name,
            Agent_Maps.First_Element (C.Agents).Get_Name);
   ...
   procedure Remove_From_All_Bags
     (This    : in out Object;
   ...
   begin
      ...
      for I in Context_Indexes.First .. Context_Indexes.Last loop
         This.Remove_From_Bag
           (Context.all,
            Bag_Key (+Context_Indexes.Vector (I)));
      end loop;
   ...
   is
      ...
      begin
         ...
         --           Log ("Bag index:" & Element (Find (Context.Bag_Indexes,
         --  Key))'Img,
   ...
   procedure Select_Random_Insertion
     (This : in Object;
      Bag  : in Bag_Key;
   ...
   function Select_Random_Context
     (This : in Object;
      Bag  : in Bag_Key)
      return Solution_Context_Ptr
   is
   ...
   is
      ...
      Pending_Tasks : Sancta.Tasks.Maps.Map;
      L             : constant Sancta.Tasks.Containers.Lists.List :=
        Sancta.Plan.Enumerate_Tasks
           (Sancta.Plan.Utils.Random.Get_Any_Expansion
               (Ass.Freeze_Plan (This.Context.Ref.Plan)),
            Primitive => True,
            Pending   => True);
      procedure Ins (I : Sancta.Tasks.Containers.Lists.Cursor) is
         use Sancta.Tasks.Containers.Lists;
      begin
         Pending_Tasks.Insert (Element (I).Get_Id, Element (I));
      end Ins;

      procedure Process_Agent (I : Sancta.Agent.Containers.Lists.Cursor) is
         use Sancta.Agent.Containers.Lists;
         use Sancta.Tasks.Containers.Lists;
         A    : constant Sancta.Agent.Object'Class          := Element (I);
         T    : constant Sancta.Tasks.Containers.Lists.List := A.Get_Tasks;
         J    : Sancta.Tasks.Containers.Lists.Cursor        := T.First;
         Prev : Sancta.Tasks.Task_Id                        := No_Task;
      begin
         --  Create its task assignment and bag
         declare
            Ag_Ctx : Agent_Context := (Solution_Context with Agent_Name =>
              +A.Get_Name);
         begin
         ...
         while Has_Element (J) loop
            if This.Context.Ref.Plan.Contains (Element (J).Get_Id)
              and then not Sancta.Plan_Node.Get_Finished
                             (This.Context.Ref.Plan.Get_Node
                                 (Element (J).Get_Id))
            then
               declare
                  ...
                  is
                     Plan : Sancta.Plan.Object renames This.Context.Ref.Plan;
                     Nx   : constant Task_Lists.Cursor := Next (K);
                  begin
                     if not Has_Element (Nx) then
                        return No_Task;
                     elsif Plan.Contains (Element (Nx).Get_Id)
                       and then not Sancta.Plan_Node.Get_Finished
               ...
               begin
                  ...
                  Prev   := C.Job;
                  C.Next := Next_Id (J);
                  Set_Attribute (C'Access, Owner, Sancta.Agent.Get_Name (A));
                  ...
                  This.Contexts.Insert
                    (Solution_Context_Key (Task_Key (C.Job)),
                     C);
               ...
               Log
                 ("Set assignment: discarding unplanned task",
                  Debug,
                  Log_Section);
            end if;
            Next (J);
      ...
      procedure Remove_Agent_Tasks
        (I : Sancta.Agent.Containers.Lists.Cursor)
      is
         use Sancta.Agent.Containers.Lists;
         use Sancta.Tasks.Containers.Lists;
         A : constant Sancta.Agent.Object'Class := Element (I);
   ...
   begin
      ...
      declare
         Agents : constant Sancta.Agent.Containers.Lists.List :=
           Ass.Get_Agents;
      begin
         Sancta.Agent.Containers.Lists.Iterate
           (Agents,
            Remove_Agent_Tasks'Access);
      end;

      --  At this point, the Pending_Tasks lists contains only tasks in the
      --  plan
      --  that weren't in the assignment received. We are going to assign them
      --  in some greedy fashion.

      Log
        ("There are" & Pending_Tasks.Length'Img & " new pending tasks.",
         Debug,
         Detail_Section);
      ...
      while not Pending_Tasks.Is_Empty loop
         declare
            New_New_Ass : Sancta.Assignment.Object;
            Success     : Boolean;
         begin
            pragma Assert (New_New_Ass.Is_Valid);
            --              Log (This.Context.Ref.Agents.Length'Img, Always);
            --              Log (Sancta.Tasks.Maps.First_Element
            --(Pending_Tasks).To_String, Always);
            Tasks.Insertions.Greedy
              (New_Ass,
               Sancta.Tasks.Maps.First_Element (Pending_Tasks),
               This.Context.Ref.Costs.Ref.all,
               Criterion,
               New_New_Ass,
               Success);
      ...
      Log
        ("Set_Assignment: Unassigned tasks inserted",
         Debug,
         Section => Detail_Section);
      ...
      declare
         Agents : constant Sancta.Agent.Containers.Lists.List :=
           New_Ass.Get_Agents;
      begin
         Sancta.Agent.Containers.Lists.Iterate (Agents, Process_Agent'Access);
      end;

      Log
        ("Set_Assignment: Assigned tasks reinserted",
         Debug,
         Section => Detail_Section);
      ...
      Log
        ("Set_Assignment: Costs reevaluated",
         Debug,
         Section => Detail_Section);
   ...
   begin
      if Assign then
         ...
         if Was_Valid and not Prev_Ass.Is_Valid then
            ...
            raise Constraint_Error with
"Will not be able to assign from invalid assignment";
         end if;
      end if;
      Clear_Dynamic_Part (This);
      C.Plan := Sancta.Plan.Inflate (Plan);
      if Assign then
         This.Set_Assignment (Prev_Ass, This.Context.Ref.Criterion);
      end if;
   end Set_Tasks;
   ...
   function Task_Key
     (Id   : in Sancta.Tasks.Task_Id)
      return Solution_Context_Key
   is
   begin
      return Solution_Context_Key'(Key ("T:" & Id'Img));
   end Task_Key;
   ...
   is
      ...
      procedure Assign_Agent (I : Agent_Maps.Cursor) is
         Agent : constant Sancta.Agent.Object'Class := Agent_Maps.Element (I);
         Curr  : Task_Context_Ptr                   :=
           Find_First (Agent.Get_Name);
      begin
         ...
         while Curr /= null loop
            Assignment.Add
              (Result,
               Agent,
               Sancta.Plan.Get_Task (This.Context.Ref.Plan, Curr.Job).all);
   ...
   begin
      ...
      Log
        ("Converted to assignment; Valid: " & This.Valid'Img,
         Debug,
         Log_Section);
      return Result;
   end To_Assignment;
   ...
   procedure Undo_From_Scratch (This : in out Object; Undo : in Undo_Info) is
   begin
   ...
   function Evaluate_Cost_Inserting
     (This                : in Object;
      ...
      Curr_To_Be_Inserted : in Sancta.Tasks.Task_Id;
   ...
   is
      ...
      Curr : Sancta.Tasks.Task_Id renames Curr_To_Be_Inserted;
      ...
      Pr, Ne : Sancta.Tasks.Task_Id := No_Task;
   begin
      ...
      Plus_1 := Cost_Cache.Get_Cost (Cm, New_Owner, Pr, Curr);

      if Next /= null then
         Plus_2 := Cost_Cache.Get_Cost (Cm, New_Owner, Curr, Ne);

         Minus := Cost_Cache.Get_Cost (Cm, New_Owner, Pr, Ne);
      end if;

      Log
        ("Inserting: [Plus1/Plus2/Minus] = " &
         To_String (Plus_1, 10) &
         " " &
         To_String (Plus_2, 10) &
         " " &
      ...
      if Plus_1 = Sancta.Infinite or else Plus_2 = Sancta.Infinite then
         return Sancta.Infinite;
      elsif Minus = Sancta.Infinite then
   ...
   exception
      when E : others =>
         Log
           ("Evaluate_Cost_Inserting [Plus1/Plus2/Minus]: " &
            Criteria.Image (Plus_1) &
            " " &
            Criteria.Image (Plus_2) &
            " " &
            Criteria.Image (Minus),
            Error,
            Log_Section);
   ...
   is
      ...
      Minus  : Costs             := 0.0;
      Cost   : Costs             :=
        Acm.Element (This.Agent_Costs.Find (Agent));
      Cm     : Cost_Cache.Object'Class renames This.Context.Ref.Costs.Ref.all;
   ...
   begin
      ...
      Plus_1 := Cost_Cache.Get_Cost (Cm, New_Owner, Pr, Curr.Job);

      if Next /= null then
         Plus_2 := Cost_Cache.Get_Cost (Cm, New_Owner, Curr.Job, Ne);
   ...
   begin
      ...
      Minus_1 := Cost_Cache.Get_Cost (Cm, String (Agent), Pr, Curr.Job);

      Minus_2 := Cost_Cache.Get_Cost (Cm, String (Agent), Curr.Job, Ne);

      Plus := Cost_Cache.Get_Cost (Cm, String (Agent), Pr, Ne);
