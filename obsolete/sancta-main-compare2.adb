      ...
      begin
         ...
         begin
            ...
            for I in 1 .. N_Done loop
               Put_Line ("I value:" & I'Img);
               Acum := Acum + Sancta.Costs (Costs (City (Tour, 1, I), City (Tour, 1, I + 1)));
               --  Remove visited job:
               declare
                  Del : Cursor := Jobs_B.Find (Get_At (Jobs, Positive (City (Tour, 1, I + 1))));
--                    J : Cursor := Jobs_B.First;
               begin
--                    Put_Line ("Deleting job: " & City (Tour, 1, I + 1)'Img);
--                    Put_Line (Has_Element (Del)'Img);
--                    Put_Line ("Target: " & Get_At (Jobs, Positive (City (Tour, 1, I + 1))).To_String);
--                    while Has_Element (J) loop
--                       Put_Line ("Candidate: " & Element (J).To_String);
--                       Next (J);
--                    end loop;
                  Jobs_B.Delete (Del);
               end;
...
begin
   ...
   if Argument_Count /= 3 then
      Put_Line ("Usage: " & Command_Name & " <Initial tasks> <Additional tasks> <window size>");
      return;
   end if;
...
--                           "[Sol] Pure Greedy plan:" &
--                           (+Cost_Greedy (Job, Jobs, Remain, TSPend => False)));
--
--                 Put_Line (Standard_Error,
--                           "[Sol] Greedy+TSP plan :" &
--                           (+Cost_Greedy (Job, Jobs, Remain, TSPend => True)));
--
--                 Put_Line (Standard_Error,
--                           "[Sol] TSP window 1    :" &
--                           (+Cost_TSP (Job, Jobs, Remain, Window => 1)));

--                 Put_Line (Standard_Error,
--                           "[Sol] TSP window" & Window_Size'Img & ":" &
--                           (+Cost_TSP (Job, Jobs, Remain, Window_Size)));
            end if;
         end;
      end loop;
   end;
